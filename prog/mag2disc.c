...
at EOF (else a rewind of the tape is starting). Included a "jump over filemark" on 
in-file. Defaults is 32768 blocks. We call 1 (Oslo)record = 4x32768 bytes.
Since exabyte records are only 32768, we read 4 times per (Oslo)records (because 
our VME system operates with buffers of 32kWords=4x32kbytes).
...
/* check commmand line args data file must be passed on command line */ 
  ...
  while (--argc > 0 && (*++argv)[0] == '-') { 
    ...
    OPTARG = FALSE;                /* clear option argument flag */ 
                                ...
                                /* open output file */ 
    OUTPUT = TRUE;
    for(n=0; n<1000; n++){ /*Finding first filenumber not used, create new file*/
       number2string(n, s1);
...
void t_help(int cond) { 
  ...
  if (cond == 1) { /* illegal command line syntax */ 
    ...
    printf(" mag2disc /dev/rmt/0 (or 1)\n"); 
