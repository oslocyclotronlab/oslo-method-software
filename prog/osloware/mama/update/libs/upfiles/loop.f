...
C Update mode = 10   10 sec. between each update
      ...
      minTime=4                                     !min freezing time
      maxTime=10                                    !max freezing time
...
C Checks if started or stopped
        ...
        IF(iD.EQ.1)OPEN(UNIT=17,FILE='/user/schiller/osloware/system/engine.lock',STATUS='OLD',ERR=99)
        IF(iD.EQ.2)OPEN(UNIT=17,FILE='/user/schiller/osloware/offline/system/offline.lock',STATUS='OLD',ERR=99)
        GO TO 98
 99     istartstop=0                    !is stopped
 98     CLOSE(17)
        IF(laststartstop.EQ.0.AND.istartstop.EQ.0.AND.l.GT.1)THEN    !stop  -> stop
          nostop=nostop+1
          IF(nostop.GT.1)system=sleep(5)
          tt=time()
          dDead=tt-tStop
          tDead=tDead+dDead
          tStop=tt
...
C Change y (z-scale) if incrementation of more than a factor of 2
        ...
        IF(l.GT.1) system=sleep(iTime)       !waiting to update...zzz...
...
C Display spectrum
        ...
        IF(l.GT.1)THEN                          !calculate how long to wait for new refresh
          dTime   =MAX0(tNew-tOld-dDead,1)
          dCounts =MAX0(cNew-cOld,0.0001)
          cAve    =(cNew+cOld)/2.0
          rate    =FLOAT(dCounts)/FLOAT(dTime)
        ...
        iDiff1=cNew
        iDiff2=max0(tNew-iT0-tDead,0)
        averate=0.0
        IF(iDiff2.GT.0.)averate=FLOAT((MAX0(cNew-iC0,0)))/FLOAT(iDiff2)
