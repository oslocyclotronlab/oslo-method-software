...
C j is destination IDEST or source ISP spectrum
      ...
      read(7,*,err=666)IYAXIS,LDX,HDX,LDY,HDY,LDZ,HDZ,LOCH,HICH,LOCNT,HICNT
      read(7,*,err=666)XDIM,YDIM,MAXCH
      read(7,*,err=666)I3,iRC,m1,m2,Idistype,OLlow,OLhigh,OLlocnt,OLhicnt
      read(7,*,err=666)Limit(0:19),COLORMAP(1:20)
      read(7,*,err=666)Istatus,ITYPE,IDEST,iCE,itext
      read(7,*,err=666)cal
      read(7,1,err=666)UNITx,UNITy,fname
...
C ixy = 0, 1, 2 means no, yes: x (1) or y (2) projection
 ...
 22     IF(ixy.GT.0)THEN                             ! the spectrum has been projected
          i1=0
          do j1=min0(62-i,60),min0(67-i,60)          !reading low limit
            IF(comm(i12,IDEST)(j1:j1).EQ.'-'.OR.comm(i12,IDEST)(j1:j1).EQ.' ')GOTO 33
            read(comm(i12,IDEST)(j1:j1),10,err=33)ii
            i1=i1*10+ii
          enddo
 33       i2=0
          do j2=min0(j1+1,60),min0(j1+4,60) !reading higher limit
...
C by definition in first spectrum - and it is always a matrix
      ...
      IF(iD.EQ.0)THEN
        lastno = lnblnk(comm(1,1))
        WRITE(6,*)'The update spectrum is neither a SIRIUS (SD) nor an' 
        WRITE(6,*)'OFFLINE (OD) spectrum. It has the history: '
        WRITE(6,*)comm(1,1)(1:lastno)
        STOP
      ENDIF
...
13      specna='NASP'                              ! finding proper name
        ...
        IF(iD.EQ.2)THEN
          IF(fil(1:4).EQ.'ALFN')specna='ALFNA'     ! only for offline
          IF(fil(1:4).EQ.'ALFG')specna='ALFGE'
          IF(fil(1:4).EQ.'XMAT')specna='XMAT'
        ENDIF
      ENDIF
