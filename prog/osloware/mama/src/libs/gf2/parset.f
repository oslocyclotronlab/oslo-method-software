...
20       X=X+PPOS(I)
      X=X/FLOAT(NPKS)
      IF (IFIXED(1).EQ.1) PARS(1)=(MSPEC(IDEST,IHI-1)+MSPEC(IDEST,ILO-1))/2.0
      IF (IFIXED(2).EQ.1) PARS(2)=(MSPEC(IDEST,IHI-1)-MSPEC(IDEST,ILO-1))/FLOAT(IHI-ILO)
