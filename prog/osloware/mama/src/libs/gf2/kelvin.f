...
C New estimate from gamma-energies - if known (1MeV = 6% and 8MeV = 3 %)
        ...
        ENDIF
      ELSE
        IXL=FLOAT((MAXCH+1.)/10.)+.5
        IXH=FLOAT((MAXCH+1.)-IXL)
        IF(IXL.LT.1)IXL=1
        IF(IXH.LT.1)IXH=1
        IYL=0
        IYH=0
...
C CALCULATING F(Ex)
      ...
      DO J=JLOWER,JUPPER
        ...
        EXH=EX+FWXG/2.
        IF(EXL.LT.0)EXL=0.
        IMAX=FLOAT((EX-EXMIN-AX0)/AX1+0.5)
        FEX(J)=0.
        GAS=0.
        GSB=0.
        DO I=0,IMAX
      ...
      DO J=JLOWER,JUPPER
        DO I=ILOWER,IUPPER
          ...
          IF(XX.GT.0.AND.XX.LT.511.)THEN
            I1=XX
            I2=I1+1
            FAC=FEX(I1)+(FEX(I2)-FEX(I1))*FLOAT(I1-XX)
          ENDIF
          IF(FAC.LE.0)FAC=0.
