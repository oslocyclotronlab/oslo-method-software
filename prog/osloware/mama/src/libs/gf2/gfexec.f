...
C --  SIRIUS
      ...
      DATA ICMNDS/
     +    'FT','LP','WM','SW','FX','FR','MA','MP','SP','DS',
     +    'DX','DY','DZ','SC','CR','UD','DF','CL','HE','PO',
     +    'WP','CE','IC','WS','SU','SE','CT','OM','EX','RP',
     +    'FI','TX','RA','CF','RW','CC','AP','DP','HC','PF',
     +    'SD','OD','OV','OS','NF','LS','DM','RF','  ','  ',
     +    '  ','LU','SL','WI','DW','  ','ME','DE','  ','ST',
...
999   WRITE(IW,*) 'Bad command'
      n=2
      IF(IR.EQ.5)CALL GFHELP(ANS(1:2),n)
      IF (IR.NE.5) THEN
...
C       FT   LP   WM   SW   FX   FR   MA   MP   SP   DS
C       DX   DY   DZ   SC   CR   UD   DF   CL   HE   PO
C       WP   CE   IC   WS   SU   SE   CT   OM   EX   RP
C       FI   TX   RA   CF   RW   CC   AP   DP   HC   PF
C       SD   SD   OV   OS   NF   LS   DM   RF   
C            LU   SL   WI   DW        ME   DE        ST
C       RE   WR   AR   OL   UC   PC   SM   CO   PM   PA      
C       FN   RN   SH   RM   UN   FO   GR   XY   NO   CU
C       NU   AN   TF   FD   FG   EL   CA   TR   UX

C Decode real input
79    CALL REIN(ANS(3:40),NC-2,r1,r2,r3)
      GO TO 90
...
90    GO TO (
     ...
     + 5900, 5200, 5300, 5400, 5500, 5900, 1900, 5800, 5900, 6000,
...
C Change weighting mode (WM)
300   CALL WEIGHT(IDATA)
      GO TO 30

C Change starting width (SW)
400   CALL STARTWID(ANS,NC)
      ans='n'
      WRITE(6,401)ans
401   FORMAT('Reset all parameters to initial values (y/n) <',A1,'>:',$)
      CALL READA1(5,ans)
      IF(ans.EQ.'y'.OR.ans.EQ.'Y')CALL PARSET(0)
      GO TO 30
...
C Open command file for input on lu IR (CF)
...
C Create look-up table file (LU)
5200  CALL LOOKUP(ANS,NC,WINMOD)
      GO TO 30

C Create slice input file (SL)
5300  CALL SLICE(ANS,NC,WINMOD)
      GO TO 30

C Add windows to look-up table or slice file using cursor (WI)
5400  CALL ADDWIN(WINMOD,&999)
      GO TO 30

C Display windows as they are presently defined (DW)
5500  CALL DISPWIN(WINMOD,&999)
      GO TO 30

C Divide the spectrum by the detector efficiency (DE)
