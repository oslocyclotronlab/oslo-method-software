...
C     Reading discrete levels from file
      ...
      INTEGER p
      CHARACTER NucName*5,STR*20,infile*15,FILNAM*15
...
51    WRITE(6,*)'Did not find ',FILNAM,' in your directory'
      WRITE(6,*)'Taking instead /user/schiller/osloware/programs/Data/lev-162Dy'
      OPEN(11,FILE='/user/schiller/osloware/programs/Data/lev-162Dy',
     1STATUS='OLD',ERR=98)
50    CONTINUE
...
4       FORMAT('Standard deviation K0 of K-distribution           <',F3.1,'>:',$)
      ...
      DO J=0,12
        ...
        DO K=0,MIN0(j,Kmax)
          IF(K_distr.EQ.0)THEN
            DisK(J,K)=exp(-FLOAT(K*K)/FLOAT(2.*K0*K0))
