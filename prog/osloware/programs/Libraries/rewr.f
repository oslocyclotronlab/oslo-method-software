...
C*******************************************************************
C
C  WRITE-PROCEDURE FOR 1 OR 2 DIMENSIONAL SPECTRA
C  Version 20. mai 1998
C*******************************************************************
...
9999  WRITE(6,*)'No file access'
  ...
  99  CONTINUE
      ...
      CHARACTER FILNAM*80,FILNAMX*80,APP*4,text*110
      REAL Calib(6), Spec(0:8191)
...
C Checks if it is a PAW file - just an array of numbers
      kch=0
      DO k=0,(4096*512)-1
...
C Transform from Intel to Sparc representation of INTEGER number
 ...
 40   FORMAT(/'ORTEC MCA singles spectrum:')
     ...
     1lvetme/50,srttme,srtsec,srtdte,strtch,lngtdt
 41   FORMAT('Type=',I4,' mca #',I2,' Segment # ',
     1I3,/,'Realtime= ',I10,' seconds, Livetime= ',
     2I10,' seconds',/,'Data collected at ',2A1,':',2A1,
...
100     FORMAT(A110)
 ...
 90     IF(NoOutput.NE.1)WRITE(6,*)text(2:iii)
        IF(text(1:12).EQ.'!CALIBRATION')THEN
          READ(text(20:62),105,ERR=106)dum,Calib(1),dum,Calib(2),dum,Calib(3)
...
100     FORMAT(A110)
 ...
 90     WRITE(6,*)text(2:iii)
     ...
     +    dum,cal(1,IDEST,1,1),dum,cal(1,IDEST,1,2),dum,cal(1,IDEST,1,3),
     +    dum,cal(1,IDEST,2,1),dum,cal(1,IDEST,2,2),dum,cal(1,IDEST,2,3)
