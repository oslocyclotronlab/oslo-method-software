 ...
 15   FORMAT('If the nucleus ',I3,'-',A2,' is not listed there, find another reference') 
      WRITE(6,'(A40)')'Very useful is e.g. the RIPL compilation'
      D=2.67
      WRITE(6,16)D
 ...
 23      FORMAT('Note: Take the row with ',I3,'-',A2,' on the left')
         WRITE(6,'(A45)')'Otherwise, use the von Egidy parameterization'
         density=A/8.
         WRITE(6,24)density
 24      FORMAT('Level density parameter a in 1/MeV <',F5.2,'>:',$)
         ...
         WRITE(6,'(A65)')'Note: For odd values of Z and N, P(Z) and P(N) are 0 respectively'
         WRITE(6,'(A56)')'Otherwise, you should use the Dobaczewski pairing energy'
         WRITE(6,'(A58)')'minus half the backshift of the von Egidy parameterization'
 ...
 25      FORMAT('Proton pairing energy P(Z=',I3,') in MeV <',F4.2,'>:',$)
         CALL READF(5,PZ)
         WRITE(6,'(A65)')'Here too, you can use the Dobaczewski-half of the von Egidy value'
