      ...
      REAL*16 D,T,epsilon,Arot,Avib,attn,xn
      REAL*16 A
      REAL*16 S,F
      REAL*16 Rf(1:1000),rho4
      REAL Starget(1:111),RhoE(1:1000,1:3)
      REAL*16 x(1:111,1:3),y(1:111,1:3),ss
...
C Calculates Rho(<E>)
      ...
      DO ii=1,3
         ...
         DO i = 1,111
            ...
            T = T - 5.
            TT = (T/1000.)**2.
            y(i,ii) = qexp(ss) / sqrt(2.*3.14*cc*TT)
            iT = T
...
C Interpolating x=<E> and y=Rho with spline-method (From the book: Numerical Recepies)
         ...
         DO i=1,NP
           xa(i)=x(i+ibase,ii)
           ya(i)=qlog(y(i+ibase,ii))       !Better to interpolate ln      
         ENDDO
         ...
         DO i=1,1000
            ...
            rho4  = rho
            Rf(i) = qexp(rho4)               ! Initial Rho assumed 
...
C Calculates Cv at T=300 and 900 keV, used to subtract linear background in next loops
  ...
  48     FORMAT(I8,5F8.1)
      ...
      REAL*16 Z,Zrot,Zvib
      REAL*16 D,T,epsilon,Arot,Avib,attn,xn
      REAL*16 r2,r3,r4,r5,r6,r7,r8,r9,r10
      REAL*16 Z0a,Z1a,Z2a,Z3a,Z4a,Z5a,Z6a,Z7a,Z8a,Z9a,Z10a,Z11a,Z12a,Z13a,Z14a,Z15a,Z16a,Z17a,Z18a,Z19a,Z20a,Z21a
      REAL*16 Z0b,Z1b,Z2b,Z3b,Z4b,Z5b,Z6b,Z7b,Z8b,Z9b,Z10b,Z11b,Z12b,Z13b,Z14b,Z15b,Z16b,Z17b,Z18b,Z19b,Z20b,Z21b
      REAL*16 Z1,Z2,Z3,Z4,Z5,Z6,Z7,Z8,Z9,Z10,Z11,Z12,Z13,Z14,Z15,Z16,Z17,Z18,Z19,Z20,Z21
      REAL*16 Z2bar,Z4bar,Z6bar,Z8bar,Z10bar,Z12bar,Z14bar,Z16bar,Z18bar,Z20bar,Z22bar
      ...
      Z0a =1
      Z1a =Z0a *1.          /(1.-qexp(-1 .*A))
      Z2a =Z1a *qexp(-1. *A)/(1.-qexp(-2. *A))
      Z3a =Z2a *qexp(-2. *A)/(1.-qexp(-3. *A))
      Z4a =Z3a *qexp(-3. *A)/(1.-qexp(-4. *A))
      Z5a =Z4a *qexp(-4. *A)/(1.-qexp(-5. *A))
      Z6a =Z5a *qexp(-5. *A)/(1.-qexp(-6. *A))
      Z7a =Z6a *qexp(-6. *A)/(1.-qexp(-7. *A))
      Z8a =Z7a *qexp(-7. *A)/(1.-qexp(-8. *A))
      Z9a =Z8a *qexp(-8. *A)/(1.-qexp(-9. *A))
      Z10a=Z9a *qexp(-9. *A)/(1.-qexp(-10.*A))
      Z11a=Z10a*qexp(-10.*A)/(1.-qexp(-11.*A))
      Z12a=Z11a*qexp(-11.*A)/(1.-qexp(-12.*A))
      Z13a=Z12a*qexp(-12.*A)/(1.-qexp(-13.*A))
      Z14a=Z13a*qexp(-13.*A)/(1.-qexp(-14.*A))
      Z15a=Z14a*qexp(-14.*A)/(1.-qexp(-15.*A))
      Z16a=Z15a*qexp(-15.*A)/(1.-qexp(-16.*A))
      Z17a=Z16a*qexp(-16.*A)/(1.-qexp(-17.*A))
      Z18a=Z17a*qexp(-17.*A)/(1.-qexp(-18.*A))
      Z19a=Z18a*qexp(-18.*A)/(1.-qexp(-19.*A))
      Z20a=Z19a*qexp(-19.*A)/(1.-qexp(-20.*A))
      Z21a=Z20a*qexp(-20.*A)/(1.-qexp(-21.*A))
      ...
      Z20b=2.*Z20a+2.*Z1a*Z19a+2.*Z2a*Z18a+2.*Z3a*Z17a+2.*Z4a*Z16a+2.*Z5a*Z15a+2.*Z6a*Z14a+2.*Z7a*Z13a+2.*Z8a*Z12a+2.*Z9a*Z11a+1.*Z10a*Z10a
      Z21b=2.*Z21a+2.*Z1a*Z20a+2.*Z2a*Z19a+2.*Z3a*Z18a+2.*Z4a*Z17a+2.*Z5a*Z16a+2.*Z6a*Z15a+2.*Z7a*Z14a+2.*Z8a*Z13a+2.*Z9a*Z12a+2.*Z10a*Z11a
...
C Attenuation (Andreas)
      ...
      IF(i.EQ.1)THEN
         Z=1.                           +p(1) *(qexp(-2.    *D/T))*Z2 +p(2)*(qexp(-2.*r2*D/T))*Z4  +
     1       p(3)*(qexp(-2.*r3*D/T))*Z6 +p(4) *(qexp(-2. *r4*D/T))*Z8 +p(5)*(qexp(-2.*r5*D/T))*Z10 +
     2       p(6)*(qexp(-2.*r6*D/T))*Z12+p(7) *(qexp(-2.*r7*D/T))*Z14 +p(8)*(qexp(-2.*r8*D/T))*Z16 +
     3       p(9)*(qexp(-2.*r9*D/T))*Z18+p(10)*(qexp(-2.*r10*D/T))*Z20 
         xn= 0                               +2.*p(1) *(qexp(-2.    *D/T))*Z2 + 4.*p(2)*(qexp(-2.*r2*D/T))*Z4  +
     1       6. *p(3)*(qexp(-2.*r3*D/T))*Z6  +8.*p(4) *(qexp(-2. *r4*D/T))*Z8 +10.*p(5)*(qexp(-2.*r5*D/T))*Z10 +
     2       12.*p(6)*(qexp(-2.*r6*D/T))*Z12+14.*p(7) *(qexp(-2.*r7*D/T))*Z14 +16.*p(8)*(qexp(-2.*r8*D/T))*Z16 +
     3       18.*p(9)*(qexp(-2.*r9*D/T))*Z18+20.*p(10)*(qexp(-2.*r10*D/T))*Z20
         xn = xn/Z
      ENDIF

      IF(i.EQ.2)THEN
         Z=Z1                           +p(1) *(qexp(-2.    *D/T))*Z3 +p(2)*(qexp(-2.*r2*D/T))*Z5  +
     1       p(3)*(qexp(-2.*r3*D/T))*Z7 +p(4) *(qexp(-2. *r4*D/T))*Z9 +p(5)*(qexp(-2.*r5*D/T))*Z11 +
     2       p(6)*(qexp(-2.*r6*D/T))*Z13+p(7) *(qexp(-2.*r7*D/T))*Z15 +p(8)*(qexp(-2.*r8*D/T))*Z17 +
     3       p(9)*(qexp(-2.*r9*D/T))*Z19+p(10)*(qexp(-2.*r10*D/T))*Z21
         xn= 1.*Z1                           +3.*p(1) *(qexp(-2.    *D/T))*Z3 + 5.*p(2)*(qexp(-2.*r2*D/T))*Z5  +
     1       7. *p(3)*(qexp(-2.*r3*D/T))*Z7  +9.*p(4) *(qexp(-2. *r4*D/T))*Z9 +11.*p(5)*(qexp(-2.*r5*D/T))*Z11 +
     2       13.*p(6)*(qexp(-2.*r6*D/T))*Z13+15.*p(7) *(qexp(-2.*r7*D/T))*Z15 +17.*p(8)*(qexp(-2.*r8*D/T))*Z17 +
     3       19.*p(9)*(qexp(-2.*r9*D/T))*Z19+21.*p(10)*(qexp(-2.*r10*D/T))*Z21
         xn = xn/Z
      ENDIF

      IF(i.EQ.3)THEN
         Z=Z2bar                           +p(1) *(qexp(-2.   *D/T))*Z4bar +p(2)*(qexp(-2.*r2*D/T))*Z6bar  +
     1       p(3)*(qexp(-2.*r3*D/T))*Z8bar +p(4) *(qexp(-2.*r4*D/T))*Z10bar+p(5)*(qexp(-2.*r5*D/T))*Z12bar +
     2       p(6)*(qexp(-2.*r6*D/T))*Z14bar+p(7) *(qexp(-2.*r7*D/T))*Z16bar+p(8)*(qexp(-2.*r8*D/T))*Z18bar +
     3       p(9)*(qexp(-2.*r9*D/T))*Z20bar+p(10)*(qexp(-2.*r10*D/T))*Z22bar
         xn= 2.*Z2bar                            +4.*p(1)*(qexp(-2.    *D/T))*Z4bar  +6.*p(2)*(qexp(-2.*r2*D/T))*Z6bar  +
     1       8. *p(3)*(qexp(-2.*r3*D/T))*Z8bar +10.*p(4) *(qexp(-2. *r4*D/T))*Z10bar+12.*p(5)*(qexp(-2.*r5*D/T))*Z12bar +
     2       14.*p(6)*(qexp(-2.*r6*D/T))*Z14bar+16.*p(7) *(qexp(-2. *r7*D/T))*Z16bar+18.*p(8)*(qexp(-2.*r8*D/T))*Z18bar +
     3       20.*p(9)*(qexp(-2.*r9*D/T))*Z20bar+22.*p(10)*(qexp(-2.*r10*D/T))*Z22bar
         xn = xn/Z
      ENDIF

      IF(i.EQ.4)THEN
         Z= (1.- (qexp(-12.*D/T))*(Z1)**12) /  (1.- (qexp(-2.*D/T))*(Z1)**2)
      ENDIF
      IF(i.EQ.5)THEN
         Z=((1.- (qexp(-12.*D/T))*(Z1)**12) /  (1.- (qexp(-2.*D/T))*(Z1)**2))*Z1
      ENDIF      
      IF(i.EQ.6)THEN
         Z=((1.- (qexp(-12.*D/T))*(Z1)**12) /  (1.- (qexp(-2.*D/T))*(Z1)**2))*Z1*Z1
      ENDIF

C Includes rotation, only R = 0,2,4,6,8,10,12
      Zrot = 1.+qexp(-Arot*2.*3./T)+qexp(-Arot*4.*5./T)+qexp(-Arot*6.*7./T) +
     1         +qexp(-Arot*8.*9./T)+qexp(-Arot*10.*11./T)+qexp(-Arot*12.*13./T)
c     Zrot = QSQRT(3.14*T/Arot)
      Z    = Z*Zrot

C Includes vibration, only v = 0,1,2
      Zvib = 1.+3.*qexp(-1.*Avib/T)     !+9.*qexp(-2.*Avib/T) +  3=beta,gamma,octupole
c     1         27.*qexp(-3.*Avib/T)+81.*qexp(-4.*Avib/T) 
      Z    = Z*Zvib
 ...
 99   RETURN
      ...
      REAL*16 D,T,epsilon,Arot,Avib,attn,xn
      REAL*16 A
      REAL*16 dT,T1,T2,Tsave,A1,A2,F,F1,F2,S
      Tsave=T 
...
C Transform S to rho by saddle point approximation
      REAL*16 Rout(1:1000), ss, cc, aa, y(1:111), rho4
...
C Finding Cv(T) and Rho(T)
      DO it = 1,NP
         ...
         TT = (T/1000.)**2.
         aa = 2. * 3.14 * cc * TT
         y(it) = QEXP(ss)/QSQRT(aa)
         IF(y(it).GT.1.D+50) GO TO 888
...
c           write(6,*)'NP,ibase',NP,ibase
      ...
      DO i=1, NP
        xa(i)=x(i+ibase)
        ya(i)=qlog(y(i+ibase))       !Better to interpolate ln    
c           write(6,*)xa(i),ya(i)  
      ENDDO
      yp1=0               !Calculates the derivatives at end-points
      ypn=0
      ...
      DO iE = 1,1000
         ...
         rho4 = rho
         Rout(iE)=qexp(rho4)
         IF(Rout(iE).GT.1.D+50) GO TO 999
...
C Transform rho to S by Laplace transformation
      REAL*16 Rin(1:1000), xx, Z0, Zl, Zh
      ...
      DO it = 1,111
         ...
         DO iE = 1,1000
            ...
            Z0 = Z0 + Rin(iE) * QEXP(-xx) * Estep/1000.
         ENDDO
         ...
         DO iE = 1,1000
            ...
            IF(xx. LT. 0.001)xx = 0.001
            IF(xx. GT. 1000.)xx = 1000.
            Zl = Zl + Rin(iE) * QEXP(-xx) * Estep/1000.
         ENDDO
         Fl = -Tl * QLOG(Zl)
         ...
         DO iE = 1,1000
            ...
            IF(xx. LT. 0.001)xx = 0.001
            IF(xx. GT. 1000.)xx = 1000.
            Zh = Zh + Rin(iE) * QEXP(-xx) * Estep/1000.
         ENDDO
         Fh = -Th * QLOG(Zh)
