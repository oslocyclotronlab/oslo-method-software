...
C Reading rhosp.rsg matrix from rhosig
      IDEST=1
      ITYPE=3
      WRITE(6,*)'Please, answer 1 and the name of your input rhosp.rsg'
...
c40    FORMAT('Microcanonical S, dS, <T>, d<T>, Cv and dCv written to: ',A11)
...
C Reading in semiexperimental rho-function - if required
      ...
      WRITE(6,*)'The spectrum is called rhotmosp.rsg and is produced by'
      WRITE(6,*)'the rhosigchi program. If you want another rho-function,'
      WRITE(6,*)'this may be created from the experimental rho-function'
      WRITE(6,*)'given in rhosp.rsg (ch 0) with a Fermi gas extension.'
      WRITE(6,*)'This can be tailored with mama - using the fit-data (FD)'
      WRITE(6,*)'command with option 5.'
           
      WRITE(6,*)' '
      WRITE(6,*)'Please, answer 1 and the name of your extrapolated singles '
      WRITE(6,*)'spectrum (rhotmosp.rsg) in the two next questions... '
      WRITE(6,*)' '
      IDEST = 1
      ITYPE = 1
      CALL READFILE
...
C Interpolating with spline-method (From the book: Numerical Recepies)
      ...
      DO i=1,NP
        xl = xa(i) - 2.                     !Going +/- 2 keV out from T 
