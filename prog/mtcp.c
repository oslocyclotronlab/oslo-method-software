/**************************************************************************** 
     ...
     mtcp.c - copy tape to tape, preserving file marks                 
            - version 1.1 J.R. Hughes LLNL 10/93 
 
     usage: mtcp [-aAs] [-b x] [-B x] [-f x] [-h x] [-n x] inTape [outTape]  
     ...
     -A     position output tape for appending prior to any           
             writes def=FALSE                                         
     -b     input blocksize in bytes def=16384 
     ...
     if no output is specified mtcp reads input, and                  
...
-------------------  
  ...
  changed default buffer size IBUFSIZ from 32768 to 16384. Translates into 
  a 35% performance gain on a 16k tape....moral - be very careful when  
...
#define DEF_BLK_SZ     16384     /* default block size for read and write */ 
...
/* check commmand line args data file must be passed on command line */ 
  ...
  printf("mtcp: %s started %s", version, datim()); 
  ...
  while( (!FILOPT || (num_files   < files_to_read)) &&  
	...
	exit(1); 
      ...
      num_files++; 
      printf("mtcp: File# %3d complete %10d bytes\n",  
	     num_files, fil_num_byt); 
...
/* print statistics */ 
  ...
  printf("mtcp: number of files  = %10d\n", num_files); 
  printf("mtcp: number of blocks = %10u\n", tot_num_blks); 
  printf("mtcp: number of bytes  = %10u\n", tot_num_byt); 
...
void t_help(int cond) { 
  ...
  printf("\nusage: mtcp [-aAHs] [-b x] [-B x] [-f x] [-h x] [-n x]");  
  printf(" intape [outtape]\n"); 
 
  if (cond == 1) { /* illegal command line syntax */ 
    printf("type   mtcp -H for help\n\n"); 
  } 
    ...
    printf(" -b x input block size = x (in bytes) default=16384\n"); 
    ...
    printf(" if no outtape is specified mtcp reads input, and"); 
