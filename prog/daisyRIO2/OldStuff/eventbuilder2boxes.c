...
/* LynxOS. There are two message-boxes, one in A24 SRAM and one in A32 DRAM.  */
/* The message box in A24 slave memory has base address pmes in               */
...
/* (second CPU). The second message-box is put after the two big buffers      */
/* The vad address is transferred to Sun via the message box.                 */
/* The second CPU (Sun CPU) access the A24 and A32 slave memories of the      */
...
#define WAIT(x) {int iii;for(iii=x;iii;iii--);}    /* 1 loop = 0.1647us */
...
#define MESSNUM        2                  /*Number of words in message box    */
...
#define SAFE           200                /*Maximum eventlength with 5TPU`s   */
#define BUFFER1        1
#define BUFFER2        2
...
#define START          0x1
#define STOP           0x0
#define AM24           0x3d               /*Address modifier for 24 bits AM24 */
...
u_long value1, value2;
u_long *pBUFFER_ADDRESS, *pBUFFER_LENGTH, *pSEMA_1, *pSEMA_2;
u_long *pVMESTATUS, *pSUNSTATUS;
long il, header, p1;
...
int curbuf, curmax, par, para, pileup, onerun, bufwait;
...
int main() {                                      /*Interrupt handler (Ctrl_C)*/
   ...
   printf("| Modified: 09-03-1998/ 12-06-1998/ 16-12-1998               |\r\n");
   printf("|____________________________________________________________|\r\n");
   printf("\r\n");
 ...
 /*  SIRIconfig();  */
   ...
   do{
      ...
      recordsold = records;
      if(cmd == 'r'|cmd == 'o'){
         *pVMESTATUS = STOP;                       /*Set VME status to stopped*/
         time( &now );
         date = localtime( &now );
...
void Loop(){
   ...
   *pT1STATUS    = CLEAR;
   *pVMESTATUS   = START;                 /*Set VME status to started         */
   *pNIMPATTERN  = CLEAR;                 /*Reset NIM ADCs for event          */
   *pSEMA_1      = EMPTY;                 /*Buffer 1 is empty                 */
   *pSEMA_2      = EMPTY;                 /*Buffer 2 is empty                 */
...
/*         WAIT(loop500ns);*/
   ...
   /*      WAIT(loop1us); */
         ...
         if(curbuf == BUFFER1){
            ...
            *pSEMA_1     = FULL;            /*Set buffer 1 semaphore to FULL  */
            records      = records + 1;
            value1        = *pSEMA_2;
            while(value1 != EMPTY){         /*Waits for buffer 2 to be fetched*/
               bufwait = bufwait+1;
               if(bufwait == 1){
                  time( &now );
                  date = localtime( &now );
                  strftime( sSTART, 80, "%c", date );
                  printf( "Waiting for buffer 2 (%s)\n", sSTART );
               }
               usleep(10000);
               value2       = *pSUNSTATUS;
               if(value2 == 0) goto waitstate;
               value1     = *pSEMA_2;
               if(leaveloop == 'y') return;           /*Jump out of event-loop*/
            }
            if(bufwait > 0){
            ...
            *pSEMA_2     = FULL;            /*Set buffer 2 semaphore to FULL  */
            records      = records + 1;
            value1        = *pSEMA_1;
            while(value1 != EMPTY){         /*Waits for buffer 1 to be fetched*/
               bufwait = bufwait+1;
               if(bufwait == 1){
                  time( &now );
                  date = localtime( &now );
                  strftime( sSTART, 80, "%c", date );
                  printf( "Waiting for buffer 1 (%s)\n", sSTART );
               }
               usleep(10000);
               value2       = *pSUNSTATUS;
               if(value2 == 0) goto waitstate; 
...
/*         if(bit(*pSIRIDSR, 7) == 1) goto nextevent;*/
         ...
         *(pbuf+p1) = SIRIpat3;           /*Pattern 3 and 4 for future use    */
         p1         = p1 + 1;
         *(pbuf+p1) = SIRIpat4;           /*Written on tape, but not used     */
         p1         = p1 + 1;
...
void Status(){
   ...
   printf("Message box   (VME A24): 0x%x  - 0x%x\n", messlow, messhigh);
   printf("Eventbuffer 1 (VME A32): 0x%x - 0x%x\n", buf1low, buf1high);
   printf("Eventbuffer 2 (VME A32): 0x%x - 0x%x\n", buf2low, buf2high);
   printf("Message box   (VME A32): 0x%x - 0x%x\n",buf2high+4, buf2high+19);
   par=*(pmes+0);
   printf("A24Messagebox 0: Address VME  0x%x \n",swap(par));
   par=*(pmes+1);
   printf("A24Messagebox 1: Bufferlength 0x%x \n",swap(par));
   par=*pSEMA_1;
   printf("A32Messagebox 0: Semaphore 1  0x%x \n",par);
   par=*pSEMA_2;
   printf("A32Messagebox 1: Semaphore 2  0x%x \n",par);
   par=*pVMESTATUS;
   printf("A32Messagebox 2: VME status   0x%x \n",par);
   par=*pSUNSTATUS;
   printf("A32Messagebox 3: SUN status   0x%x \n",par);
...
void BUFFERopen(){
          ...
          /*in addition 4 words = 16 bytes makes 0x40010 bytes*/
   bufferbytes=0x40010;
   if(err=uio_open()) {
   ...
   if (!(mem32=(u_long)smem_create("MEM32",(char *)(ad&~0xFFF),bufferbytes, SM_READ|SM_WRITE))){
      fprintf(stderr,"Unable to allocate MEM window\n");
      exit(0);
   }
   pbuf = (u_long *)(mem32 | (ad&0xFFF));
  /* printf("Pointer in program:              0x%08x...DONE\n",pbuf); */

   pSEMA_1         = pbuf + 0x10000 + 0;                   /*New A32messagebox*/
   pSEMA_2         = pbuf + 0x10000 + 1;
   pVMESTATUS      = pbuf + 0x10000 + 2;
   pSUNSTATUS      = pbuf + 0x10000 + 3;
   printf("DONE \n");
   return;
}
...
void BUFFERclear(){
   ...
   p1         = 0;
   *pSEMA_1   = EMPTY;               /*Set semaphore for buffer to empty state*/
   *pSEMA_2   = EMPTY;
...
void MESSAGEBOXopen(){
   messagebytes = 0x8;
   printf("Allocating SRAM A24 slave memory (message box) of 0x%x
bytes...", messagebytes);
   if (!(mem24=(u_long)smem_create("MEM24",(char *)(CPUADDR&~0xFFF),messagebytes, SM_WRITE | SM_READ))){
      fprintf(stderr,"Unable to allocate MEM window for slave A24 memory");
      exit(0);
   }
   pmes = (u_long *)(mem24 | (CPUADDR&0xFFF));         /*First 2 message-words*/
   pBUFFER_ADDRESS = pmes + 0;
   pBUFFER_LENGTH  = pmes + 1;
/* pSEMA_1         = pmes + 2;
   pSEMA_2         = pmes + 3;
   pVMESTATUS      = pmes + 5;
   pSUNSTATUS      = pmes + 6;this was the old A24messagebox*/
   printf("DONE \n");
   return;
}
...
void MESSAGEBOXconfig(){
   ...
   *pVMESTATUS      = STOP;                     /*Eventbuilder is not running */
   *pSUNSTATUS      = STOP;                     /*Sirius is not running       */
   printf("DONE \n");
   return;
}
...
void MESSAGEBOXclose(){
   *pVMESTATUS      = STOP;
   ...
   smem_create("MEM24",(char *)mem24,0x8,SM_DETACH);
   smem_remove("MEM24");
   printf("DONE \n");
   return;
}
...
void TPUopen(){
 ...
 /*printf("NIM ADCs:\n");*/
   ...
   for(i=0; i<16; i++){
    /*printf("ADC no %2d, AD=0x%08x, AM=0x%02x: data read: %8x\n", i,
NIMbaseaddr+2*i, AM24, *(pNIMadc+i)); */
   }
   printf("DONE \n");
   return;
