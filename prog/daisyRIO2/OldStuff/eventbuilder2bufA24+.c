...
endret *pSIRIEVRD=ENABLE til CLEAR f¿r loop
*/
...
int main() {                                      /*Interrupt handler (Ctrl_C)*/
   ...
   printf("| Modified: 09-03-1998/ 12-06-1998/ 05-01-1999 / 21-07-1999  |\r\n");
   printf("|____________________________________________________________|\r\n");
   printf("                                                              \r\n");
...
void Loop(){
   ...
   *pSEMA_2      = swap(EMPTY);           /*Buffer 2 is empty                 */
   ...
   for(;;){
      ...
      /*  have to segment 1 eventbuffers (32 kwords) into 2 movebuffers,     */
...
void Status(){ 
   ...
   movelow   = vad;
   movehigh  = vad + 4 * (((MAXBUF+1)/2)-1);

   printf("Length of eventbuffer in RTPC8067:     0x%x bytes\n", bufferbytes);
   printf("Length of movebuffer RTCP8067 <-> Sun: 0x%x bytes\n", movebytes);
   printf("Length of messagebox RTCP8067 <-> Sun:    0x%x bytes\n", messagebytes);
   ...
   par=*(pmes+5);
   printf("Messagebox 5: Not used     0x%x \n",swap(par));
   par=*(pmes+6);
...
void BUFFERclear(){
   ...
   for( i = 0; i < ((MAXBUF+1)/2)+1; i++){                  /*Clear movebuffer*/
      *(pmove+i) = 0;
   }
...
void MESSAGEBOXconfig(){
   ...
   *(pmes+4) = 0;                          /*Zeroing unused locations         */
   *(pmes+5) = 0;
   *(pmes+8) = 0;  
   *(pmes+9) = 0;
