...
int main() {                                      /*Interrupt handler (Ctrl_C)*/
   ...
   printf("| Modified: 09-03-1998/ 12-06-1998/ 05-01-1999 / 20-04-1999  |\r\n");
   printf("|____________________________________________________________|\r\n");
   printf("                                                              \r\n");
...
void Loop(){
   ...
   *pSEMA_2      = swap(EMPTY);           /*Buffer 2 is empty                 */
   ...
   for(;;){
      ...
      *pSIRIEVRDY  = ENABLE;                     /*Reset SIRI for next event  */
    /*  pSIRICOST   = 0x81;*/
      TPUS = 0;
      SIRI = 0;
      if(leaveloop == 'y') return;               /*Jump out of event-loop     */
      while(TPUS + SIRI == 0){
         ...
         WAIT(loop500ns);
         SIRI   = !bit(*pSIRICOST, 7);           /*Bit7=0 means event         */
       /* printf("TPUS = %x \n", TPUS ); */
      }
...
void BUFFERopen(){        /*Allocation of physically contiguous memory        */
   ...
   if (!(mem32=(int)smem_create("MEM32",(char *)(ad&~0xFFF),bufferbytes, SM_WRITE))){ 
