     ...
     1A(3),DELTAA(3),SIGMAA(3),CH(1010)
      ...
      WRITE(6,*)'    |                                         |'
      WRITE(6,*)'    |          L E G E N D R E  1.0           |'
      WRITE(6,*)'    |                                         |'
      WRITE(6,*)'    |    Program to fit Legendre polynom  to  |'
      WRITE(6,*)'    |  an experimental angular distribution   |'
      WRITE(6,*)'    |      W(Theta) = a0P0 + a2P2 + a4P4      |'
      WRITE(6,*)'    |Oslo Cyclotron Laboratory, September 1996|'
      WRITE(6,*)'    |            Magne Guttormsen             |'
      WRITE(6,*)'    |_________________________________________|'


      OPEN(23,FILE='legendre.dat',STATUS='old',ERR=666)
      READ(23,*,ERR=666)NA,NPTS
      DO i=1,NPTS
        READ(23,*,ERR=666)X(i),Y(i),SIGMAY(i)
      ENDDO
      CLOSE(23)
      WRITE(6,*)'Default values taken from file: legendre.dat'
      GO TO 777
 666  NA=2
      NPTS=9
      DO i=1,NPTS
...
C
      ...
      WRITE(6,*)' '
      WRITE(6,*)' '
      WRITE(6,*)'Type the X-values (Thetas in degrees):'
      DO i=1,NPTS
        WRITE(6,41)i,X(i)
 41     FORMAT(' Type X(',I2,')      <',F12.4,'>:',$)
      ...
      WRITE(6,*)' '
      WRITE(6,*)'Type the Y-values (intensities):'
      DO i=1,NPTS
        WRITE(6,42)i,Y(i)
 42     FORMAT(' Type Y(',I2,')      <',F12.4,'>:',$)
      ...
      WRITE(6,*)' '
      WRITE(6,*)'Type the Ysig-values (standard deviations):'
      DO i=1,NPTS
 ...
 43     FORMAT(' Type Ysig(',I2,')   <',F12.4,'>:',$)
      ...
      OPEN(23,FILE='legendre.dat',ERR=555)
...
C
  ...
  78  FORMAT(' Loop    Chi**2         a0')
  79  FORMAT(' Loop    Chi**2         a0           a2')
  80  FORMAT(' Loop    Chi**2         a0           a2           a4')
...
44      FORMAT(3F12.4)
  ...
  83  FORMAT(' a0 = ',E12.5,' +/- ',E12.5)
  84  FORMAT(' a2 = ',E12.5,' +/- ',E12.5)
  85  FORMAT(' a4 = ',E12.5,' +/- ',E12.5)
...
C
   ...
   40 RETURN
      ...
      FUNC=0
      x=x*3.1415927/180.
      FUNC=A(1)+A(2)*(.5*(3.*(COS(X)**2.)-1.))+
     +A(3)*(.125*(35.*(COS(X)**4.)-30.*(COS(X)**2.)+3.))
      RETURN          
      END
