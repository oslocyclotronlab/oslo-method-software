...
/* is performed with the swap(x) macro below. For values written by Sun, the  */
/* the numbers turns out like 0xd30000707 - if 7 is written. Therefore, swap1 */
/* is constructed to solve this problem of A24. The two eventbuffers (in slave*/
...
/* For further details, see institute report of 1998...                       */


/* New untested things: 
signal.h with leaveloop
messageboxreset killed, cccc in waitstate
usleep(2) in bufferhandlingsection
usleep(10) in waitstate             
events and records
stop date and time 
Only 3840 kanaler accepted for CAMAC adcs
Included slave program
Changed all camac to end
Slave PCI registers initiated: SLAVEinitiate
Default delay of 50 us  
inbuf  
events, records in loop()
...DONE
...
unsigned short patcopy1, patcopy2, patcopy3, ius;
int loopmicrosecond = 0;                    /*Loop extra until ADC finished   */
...
int main() {                               /*Interrupt handler (Ctrl_C)       */
   ...
   printf("| Modified: 09-03-1998/ 12-06-1998/ 15-10-1998               |\r\n");
   printf("|____________________________________________________________|\r\n");
   printf("                                                              \r\n");
 ...
 /*  SIRIconfig();  */
   ...
   records =  0;
   para    = 50;
   loopmicrosecond = (int)(((float)para / usLOOP) + 0.5);
   ...
   do{
      ...
      leaveloop ='n';                           /*Do not jump out of eventloop*/
      printf("eventbuilder+>");
      cmd = readchar();
      switch (cmd) {
...
void Loop(){
   ...
   printf( "To jump out of eventloop, press Ctrl_C\n");
   ...
   *pSEMA_2      = swap(EMPTY);           /*Buffer 2 is empty                 */
   ...
   /* *pSIRIEVRDY  = ENABLE; */                  /*Enable SIRI for next event */
      ...
      WAIT(loopmicrosecond);               /*Waiting until ADCs have converted*/
      ...
      /* :::::::::::::::::::::::::::::::::::::::::::::::::::*/
      /*     Check for pileup (assumes 2 us is over)        */
      /* :::::::::::::::::::::::::::::::::::::::::::::::::::*/
      para   = *pT1NUMREG;
      ...
      if (TPUS > 0){
         ...
         if (patcopy1 != 0){ 
            ...
            *(pbuf+p1) = patcopy1;                           /*Store pattern 1*/
            ...
            for(ius = 0; ius < 15; ius+=2){
               if( bit(patcopy1, ius) == 1){ 
                  *(pbuf+p1) = *(pNIMadc+ius);
                  p1         = p1 + 1;
                  *(pbuf+p1) = *(pNIMadc+ius+1);
                  p1         = p1 + 1;
               }
            }
         }
...
void Dumpbuf(){
   ...
   printf("Percent data equal 0               %6.1f\n", pzro);
   para = (int)(usLOOP * loopmicrosecond + 0.5);
   printf("Extra delay is %d us (waitingloops = %d)\n", para, loopmicrosecond);  
   printf("\n"); 
   return;
...
void Microsecond(){
   ...
   printf("\n");
   printf("With this command you can add delay to the event loop. This\n");
   printf("may apply for ADCs with long conversion times. (If the CPU \n");
   printf("runs at another MHz, the time given is incorrect.)\n");
   para = (int)(usLOOP * loopmicrosecond + 0.5);
   printf("Extra delay is %d us (waitingloops = %d)\n", para, loopmicrosecond); 
   printf("Type microseconds (us) of delay: ");
   gets(inbuf);
   sscanf(inbuf,"%d",&para);
   loopmicrosecond = (int)(((float)para / usLOOP) + 0.5);
   para = (int) (usLOOP * loopmicrosecond + 0.5);
...
void SLAVEinitiate(){
   ...
   printf("Enable slave A24 memory via PCI_CTL registers...");
