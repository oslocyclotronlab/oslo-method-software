...
long for *pSUNSTATUS etc.
status, 3 swap1 to swap
u_long -> long for vad pmes pbuf ...
new: struct pdparam_slave sp
mem24 mem32 long -> int
*/
...
int main() {                                      /*Interrupt handler (Ctrl_C)*/
   ...
   printf("| Modified: 09-03-1998/ 12-06-1998/ 20-12-1998               |\r\n");
   printf("|____________________________________________________________|\r\n");
   printf("                                                              \r\n");
...
void SIRIclose (){
   ...
   vme_rel(pSIRICLR,   sizeof(u_ char));
   vme_rel(pSIRICOST,  sizeof(u_ char));
   vme_rel(pSIRIEVEAD, sizeof(u_ char));
   vme_rel(pSIRIBUFAD, sizeof(u_ char));
   vme_rel(pSIRIEVRDY, sizeof(u_ char));
   vme_rel(pSIRIIRQVC, sizeof(u_ char));
   vme_rel(pSIRINXBUF, sizeof(u_ char));
   vme_rel(pSIRINUBUF, sizeof(u_ char));
