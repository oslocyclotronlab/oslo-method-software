...
c      i1=14                   ! First point for fit (lowest is 1, not 0)
		i1=30                   ! First point for fit (lowest is 1, not 0)

      i2=60                  ! Last point for fit
      NPTS=i2-i1+1           ! Number of fit-points
      NA= 1                  ! Number of fit parameters
      Exmin = 4.0           ! Lower Ex for 1.gen. spectra
      Exmax = 20             ! Higher Ex for 1. gen. spectra
      Delta = 2.080          ! Delta_p+n for 96Mo
		xZ = 50
		xN = 67
      xA = 117
      beta = 0.1
...
c new Egidy values(robin). Overwriting the previous values for a and E1
      aLev = 13.62
      E1   = -0.21

C Estimate values of parameters to be fitted
      A(1)=0.5             ! K-factor in front of E1+M1
      A(2)=8.5
      A(3)=10.
      A(4)=2.0
...
c       WRITE(6,*)'Upbend modeled as a power law of the form'
c       WRITE(6,*)'K*1/(3*(pi*c*hbar)**2)*A*(E_gamma/[MeV])**-B,'
c       WRITE(6,*)'where the two parameters A and B>0,'
c       WRITE(6,*)'1/(3*(pi*c*hbar)**2)=8.673723898E-8/(mb*MeV**2),'
c       WRITE(6,*)'and the overall normalization K will be explained later.'
c       A(2)=0.5
c       A(3)=2.5
...
C START TO INCLUDE FIT WITH GDR DATA (comment away.....)
      NPTSold = NPTS		
      j1 = 1
      j2 = 10
		NPTS = j2 - j1 + 1
		OPEN(23,FILE='GDR_rsf_Sn117.dat',STATUS='old',ERR=766)
      GO TO 768
 766  WRITE(6,*) 'Sorry, could not open GDR_rsf_Sn117.dat'
      STOP
 768  DO i=1,1000
         READ(23,*,END=767)xx(i),yy(i),ss(i)
      ENDDO
 767  Ntot=i-1
      CLOSE(23)
C Putting values into the fit-vectors
      DO i=1,NPTS
        X(i+NPTSold)      = xx(i+j1-1)
        Y(i+NPTSold)      = yy(i+j1-1)
        SIGMAY(i+NPTSold) = ss(i+j1-1)
      ENDDO
      NPTS = NPTS + NPTSold
C ENDING TO INCLUDE FIT WITH GDR DATA (comment away.....)		
...
c      WRITE(6,85)  A(3),SIGMAA(3)
		...
		write(6,90) A(2), sigmaa(2)
		WRITE(6,88)  A(3),SIGMAA(3)
		WRITE(6,89)  A(4),SIGMAA(4)
		

c      WRITE(6,87)  A(2),SIGMAA(2)
c      WRITE(6,88)  A(3),SIGMAA(3)
...
c  88  FORMAT(' A(6) g2 = ',E12.5,' +/- ',E12.5)
   ...
   88  FORMAT(' A(3) Spy  = ',E12.5,' +/- ',E12.5)
   89  FORMAT(' A(4) Gpy  = ',E12.5,' +/- ',E12.5)
c	90  FORMAT(' A(2) T  = ',E12.5,' +/- ',E12.5)

c	87  FORMAT(' A(2) Epy  = ',E12.5,' +/- ',E12.5)


c   87  FORMAT(' A(2) A  = ',E12.5,' +/- ',E12.5)
c   88  FORMAT(' A(3) B  = ',E12.5,' +/- ',E12.5)
  90  FORMAT(' A(2) Epy  = ',E12.5,' +/- ',E12.5)
...
c         WRITE(24,*) e, EXP(FUNC(e,A)),fe11(e,A)+fe12(e,A),upbend(e,A),fm11(e,A),
c     1   fe2(e,A),dummy
			WRITE(24,*) e, EXP(FUNC(e,A)),fe11(e,A)+fe12(e,A),upbend(e,A),fm11(e,A),
     1   fe2(e,A),fpy11(e,A)
      ENDDO
...
c		WRITE(24,87)  A(2),SIGMAA(2)
		WRITE(24,88)  A(3),SIGMAA(3)
		WRITE(24,89)  A(4),SIGMAA(4)

c      WRITE(24,87)  A(2),SIGMAA(2)
c      WRITE(24,88)  A(3),SIGMAA(3)
...
C  DIVIDE BY NUMBER OF DEGREES OF FREEDOM
   ...
   40 RETURN
      ...
      FUNCTION FUNC(X,A)
      REAL A(12)
c      FUNC=LOG(fe11(x,A) +fe12(x,A)+ fm11(x,A) + upbend(x,A) +fe2(x,A))
		FUNC=LOG(fe11(x,A) + fe12(x,A) +fm11(x,A)  +fe2(x,A) +fpy11(x,A))
      RETURN          
      END
...
c        write(6,*)ee1,se1,ge1,ee2,se2,ge2,em1,sm1,gm1,eeq,seq,geq,aLev,E1,Exmin,Exmax
     ...
     1   (0.7*se1*ge1**2*(eg**2+4*pi**2*T**2))/(ee1*(diff1)**2) ))*dEx    ! KMF model
	  
c		fe11  = fe11+(xK*(8.6737E-08)*(
c     1    (se1*eg*ge1**2)/((eg**2-ee1**2)**2+eg**2*ge1**2)))*dEx  ! Lorentzian
		sum   = sum   +   dEx
      ENDDO
      fe11=fe11/sum
      RETURN
...
c      FUNCTION plotlint(X,A)
c      REAL A(12)
c      plotlin=fe11(x,A)+fm11(x,A)                !+fe12(x,A)+fpy11(x,A)
c      RETURN          
c      END
