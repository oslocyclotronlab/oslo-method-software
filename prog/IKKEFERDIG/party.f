      ...
      WRITE(6,*)'|                                             |'
      WRITE(6,*)'| Calibration for S(T),F(T),E(T),C(T),Rho(T): |'
...
C Tmin and Tmax do not change calibration, dT=0.03 always (magne)
      ...
      WRITE(6,*)'Default values taken from file: party.dat'
      CLOSE(10)
      GO TO 103
 101  CONTINUE
      CLOSE(10)
 ...
 112  FORMAT('Vibrational quantum energy in keV            <',F6.1,'>:',$)
      ...
      WRITE(6,113)Tminr
 113  FORMAT('Lower limit for T in MeV (min=0.03 MeV)       <',F5.3,'>:',$)
      CALL READF(5,Tminr)
      IF(Tminr.LT.0.03)STOP 'T>=0.03 required'
      WRITE(6,114)Tmaxr
...
c      ENDIF
 ...
 118  CONTINUE
      ...
      Tmin=Tminr
      Tmax=Tmaxr
C      IF(nT.GT.1)dT=(Tmax-Tmin)/DBLE(nT-1)
      dT=0.030
      IF(eo.EQ.0)THEN
         nc=4
      ELSE
 ...
 121              CONTINUE
                  ...
                  ELSE
                     ...
                     ELSE IF(atr.EQ.1.)THEN
                        pD=QEXP(DBLE((k-1)/2)*DpT)
                     ELSE
                        pD=QEXP(DBLE((1-at**((k-1)/2))/(1-at))*DpT)
                     ENDIF
                  ENDIF
                  zp=zp+pe*pD
 122           CONTINUE
 ...
 123              CONTINUE
                  ...
                  ELSE
                     IF(atr.EQ.0.)THEN
                        pD=DBLE(1)
                     ELSE IF(atr.EQ.1.)THEN
                        pD=QEXP(DBLE((k-2)/2)*DpT)
                     ELSE
                        pD=QEXP(DBLE(QSQRT(at)*(1-at**((k-2)/2))/(1-at))*DpT)
                     ENDIF
                  ENDIF
                  zp=zp+pe*pD
 124           CONTINUE
 ...
 125              CONTINUE
                  ...
                  ELSE
                     IF(atr.EQ.0.)THEN
                        nD=QEXP(DnT)
                     ELSE IF(atr.EQ.1.)THEN
                        nD=QEXP(DBLE((k-1)/2)*DnT)
                     ELSE
                        nD=QEXP(DBLE((1-at**((k-1)/2))/(1-at))*DnT)
                     ENDIF
                  ENDIF
                  zn=zn+ne*nD
 126           CONTINUE
 ...
 127              CONTINUE
                  ...
                  ELSE
                     IF(atr.EQ.0.)THEN
                        nD=DBLE(1)
                     ELSE IF(atr.EQ.1.)THEN
                        nD=QEXP(DBLE((k-2)/2)*DnT)
                     ELSE
                        nD=QEXP(DBLE(QSQRT(at)*(1-at**((k-2)/2))/(1-at))*DnT)
                     END IF
                  END IF
                  zn=zn+ne*nD
 128           CONTINUE
 ...
 129           CONTINUE
            ...
            IF(vbr.GT.0.)THEN
               vbT=-vb/T
               DO 130 k=1,1
                  Zv=Zv+DBLE(3**k)*QEXP(DBLE(k)*vbT)
 130           CONTINUE
            END IF
            Z=zp*zn*Zr*Zv
 131        CONTINUE
            ...
            IF(STnew*CTnew*T.GT.0)RhoT(j,eo)=QEXP(STnew)/QSQRT(DBLE(2.*3.14)*CTnew*T*T)
 ...
 132     CONTINUE
 133     CONTINUE
c         CALL eval(FT,nTmax,TT,ST,ET,CT,RhoT,eo) !does not interpolate nicely for T>4 MeV
         DO 135 j=1,200,1
...
C Has to calculate new nTmax, called imax, due to new standard calibration
      imax=MIN(200,(TT(nTmax,eo)-Tstart)/Tstep)
      DO i=1,imax
         T=Tstart+Tstep*(i-1)
         TT(i,eo)=T
...
C From three points (F1,F,F2), we may differentiate 1 and 2 times
      ...
      DO i=1,imax
         ...
         d1F=(F2-F1)/(2.*dT)
C         d2F=(((F2-F)/dT)-((F-F1)/dT))/dT   !or (F1+F2-2*F)/(dT**2)
         d2F=(F1+F2-2*F)/(dT**2)
