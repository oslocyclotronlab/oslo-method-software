...
C Read/write stuff (mama)
      ...
      WRITE(6,*)'    |                                                |'
      WRITE(6,*)'    |                 R A D E X  1.6.1               |'
      WRITE(6,*)'    |                                                |'
      WRITE(6,*)'    |     Program to calculate radiative strength    |'
      WRITE(6,*)'    |   functions (RSF) f(E_i) and f(E_f) when the   |'
      WRITE(6,*)'    |   level density Rho is known. The inputs are   |'
      WRITE(6,*)'    |  taken from the program rhosigchi (fg.rsg and  |'
      WRITE(6,*)'    |   fgerr.rsg) and the kumac files (rhopaw.cnt,  |' 
      WRITE(6,*)'    | sigextpaw.cnt and rhotmopaw.cnt). These files  |'
      WRITE(6,*)'    |    should reside in your current directory.    |'
      WRITE(6,*)'    | Created:  Apr   2004                           |'
      WRITE(6,*)'    | Modified: Aug   2014 include Ex < 0 MeV        |'
...
991   WRITE(6,*)infile,' read with dimension =',ich+1
      ...
      DO i=0,8192
        ...
        f(i)=xdumr
        ich=i
c        WRITE(6,*)' rhopaw.cnt value =',f(i)
...
C Finding x-markers
      ...
      ENDDO
      DO i=1,XDIM-1
         IF(projx(i-1).EQ.0.AND.projx(i).NE.0)THEN
            mxLow=i
            GO TO 5
         ENDIF
      ENDDO
5     CONTINUE
      DO i=XDIM-1,mxlow,-1
         IF(projx(i).EQ.0.AND.projx(i-1).NE.0)THEN
            mxHigh=i-1
            GO TO 7
         ENDIF
      ENDDO
...
C Finding y-markers
      ...
      ENDDO
      DO j=1,YDIM-1
         IF(projy(j-1).EQ.0.AND.projy(j).NE.0)THEN
            myLow=j
            GO TO 11
         ENDIF
      ENDDO
11    CONTINUE
      DO j=YDIM-1,mylow,-1
         IF(projy(j).EQ.0.AND.projy(j-1).NE.0)THEN
            myHigh=j-1
            GO TO 12
         ENDIF
      ENDDO
