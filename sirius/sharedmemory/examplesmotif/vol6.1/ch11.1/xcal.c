...
char *argv[];
{
    ...
    fontlist = XmFontListAdd(fontlist, font, "charset2");
    ...
    {
        XmStringTable strs =
            ArgvToXmStringTable(XtNumber(months), months);
...
XmListCallbackStruct *list_cbs;
{
    ...
    *p = 0;
    ...
    /* display the month using the "charset1" font from the
...
char **argv;
{
    ...
    new[argc] = 0;
    while (--argc >= 0)
        new[argc] = XmStringCreate(argv[argc], "charset2");
    return new;
}
