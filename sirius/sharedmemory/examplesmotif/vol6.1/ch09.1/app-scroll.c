...
/* app-scroll.c - Displays bitmaps specified on the command line.  All
 ...
 *
 * Example:
 *  app-scroll /usr/include/X11/bitmaps/*
 */
...
int rows, cols;
unsigned int
    cell_width, cell_height, pix_hoffset, pix_voffset, sw_hoffset, sw_voffset;
void redraw();
 ...
 */
void
scrolled(scrollbar, orientation, cbs)
Widget scrollbar;
 ...
 */
void
expose_resize(drawing_a, unused, cbs)
Widget drawing_a;
XtPointer unused;
...
Window window;
{
    ...
    /* Some X servers might require a call to XClearWindow() here in order
     * to clean up "dirty" parts of the ScrolledWindow's viewport.  So far,
     * this only happens on the black and white screen for Sun's with enable
     * planes (sun3/60, sun4/110, etc).
