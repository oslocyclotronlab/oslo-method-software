...
/* Define global variables */  
	...
	int     tapetype, outfileno;
        
int acq_position( int cup, int bot, int eod, int fileno, int recno)
{
	int				stor_off = 2;
	int				*storp;
	int				current_file;
	...
	char			errd[1024]="*** ERROR *** Could not open any disk sirius_0-999 in present directory";
...
/* Determining current file and record for Disk */
	...
	/* Read the status */
        ...
        if ( tapetype != 3) {              /* Case of exabyte-files */
	   mt_command.mt_op = MTNOP;
	   if (ioctl( exades, MTIOCTOP, &mt_command) == -1) {
	      return -1;
           }  
	   ...
	   if ( files2skip == 0 && recs2skip == 0 ) {
        ...
        if ( tapetype == 3) {                                   /* Case of disk-files */
