...
int main()
{
	...
	int xdim, ydim;
	char err1[1024] = "Lock file present: /Applications/sirius/system/specdump.lock";
...
/* This is a security precaution, multiple processes may cause system crash */
   if(fd=open("/Applications/sirius/system/specdump.lock",O_CREAT|O_EXCL,PERMS)==-1 )
   {
      printf("%s\n",err1);
      exit(errno);
   }
