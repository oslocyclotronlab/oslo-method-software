...
int main()
{
	/* ******************************************************/
	/*    PROGRAM    : specclear							*/
	/*    FILE       : ~/sirius/offline/src/specclear.c		*/
	...
	FILE *fp;
	int xdim, ydim;
	char err1[1024] = "Lock file present: /Applications/sirius/offline/system/specclear.lock";
...
/* This is a security precaution, multiple processes may cause system crash */
   if(fd=open("/Applications/sirius/offline/system/specclear.lock",O_CREAT|O_EXCL,PERMS)==-1 )
   {
      printf("%s\n",err1);
      exit(errno);
   }
