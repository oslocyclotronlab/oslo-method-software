...
extern char* WINDNAME;
extern float xwg_defaults[4];
...
int initg_(int *nx, int *ny)
{
  ...
  if (first) {
    ...
    if (xwg[2] == 0.0) {
      int d;
      for(d=0; d<4; ++d)
        xwg[d] = xwg_defaults[d];
    }
    ix = xwg[0]*(float)scr_width;
    iy = xwg[1]*(float)scr_height;
    w  = xwg[2]*(float)scr_width;
    h  = xwg[3]*(float)scr_height;
 ...
 /* Get named color values */
    XFlush(disp_id);      
      color[1]   = define_color("BLUE");
      color[2]   = define_color("DEEP SKY BLUE");
      color[3]   = define_color("LIGHT SKY BLUE");

      color[4]   = define_color("SEA GREEN");
      color[5]  = define_color("MEDIUM SEA GREEN");
      color[6]  = define_color("GREEN");

      color[7]  = define_color("BROWN");
      color[8]  = define_color("CHOCOLATE");
      color[9]  = define_color("SANDY BROWN");

      color[10]   = define_color("RED");
      color[11]   = define_color("CORAL");
      color[12]   = define_color("ORANGE");

      color[13]  = define_color("YELLOW3");
      color[14]  = define_color("YELLOW2");
      color[15]  = define_color("YELLOW");

      color[16]  = define_color("PEACH PUFF");
      color[17]  = define_color("PAPAYA WHIP");
      color[18]  = define_color("OLD LACE");
...
*/
    ...
    xgcvl.background = white;
    xgcvl.foreground = black;
...
int setcolor_(int *picol)
{
  int i,icol;
  icol=*picol;
  i = icol + 0;       /* before icol+1, magne*/
  if (i < 1 || i > 20) return 1;
