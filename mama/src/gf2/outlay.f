...
C Routine to display part of a matrix as a number of singles spectra
 ...
 5      FORMAT( 'Higher channel on y-axis <',I5,'>:',$)
        ...
        IF(m1.GT.2047.OR.m2.GT.2047)Istatus=2
...
C Spectrum loop starts
      DO m=m1,m2
        ...
        IF(iOL.EQ.0)THEN
          OLlow  =min(OLlc(I1),ABS(maxdim-11.0))    !Using old values
          OLhigh =max(OLhc(I1),10.0)
...
C Making axis
        ...
        IF(NCC.EQ.1)THEN
          s=Spec(OLlow+1)
                 IF(IYAXIS.EQ.3.AND.s.LT.Y0)s=Y0
          CALL PSPOT(X,s)
