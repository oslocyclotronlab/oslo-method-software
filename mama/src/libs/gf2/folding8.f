...
C Reading dimension and energy calibration
  ...
  11  FORMAT( 'Cal. coeff. a1 (keV/ch)  <',F8.1,'>:',$)
      ...
      IRSP=3
      WRITE(6,*)"Choose your response function. The ones marked old are not recommended"
      WRITE(6,2)IRSP
   2  FORMAT(/'NaI_old(1), LaBr_2012(2), NaI_2012(3), Seg2(4), Seg3(5), Clover_old(6), Clover(7), Gauss(8) <',I1,'>:',$)
      CALL READI(5,IRSP)
      IF(Istatus.NE.0)RETURN
...
C THE NUMBER OF CALIBRATION POINTS (Default is NaI)
      ...
      IF(IRSP.EQ.3)THEN         !NaI2012 (Default)
         IW  =10
         ITAB=15
         IR  =10
         DO i=1,IW
...
C Finding total efficiency as function of full energy
      ...
      IF(IRSP.LE.7)CALL RSPDetector(IRSP)
      IF(IRSP.EQ.8)CALL RSPGAUSS
...
C first time spectrum exeeds this threshold
          IF(IRSP.EQ.4.OR.IRSP.EQ.5.OR.IRSP.EQ.6.OR.IRSP.EQ.7)GO TO 1111  !Drops this for Geant spectra (seg2 and seg3 and Clover)
          maxi=(300.-b0)/b1 + 0.5
          h1=0.
          h2=0.
          ipass1=0
